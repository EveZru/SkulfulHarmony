...
public class PreguntasRecomendacion extends AppCompatActivity {
    ...
    private List<Pregunta> listaDePreguntas = new ArrayList<>(); // Inicialización
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listaDePreguntas.add(new Pregunta("¿Es la primera vez que tomarás clases de música?", respuestas1));
        ...
        listaDePreguntas.add(new Pregunta("¿Para comenzar qué instrumento te gustaría aprender a tocar?", respuestas2));
        ...
        listaDePreguntas.add(new Pregunta("¿Con qué frecuencia desea tomar clases?", respuestas3));
        ...
        listaDePreguntas.add(new Pregunta("¿Qué géneros musicales deseas aprender a tocar?", respuestas4));
...
/*
        ...
        btn_siguiente_pregunta.setOnClickListener(v -> {
            guardarRespuesta(currentPreguntaIndex); // Guarda antes de avanzar
        ...
        btn_anterior_pregunta.setOnClickListener(v -> {
            guardarRespuesta(currentPreguntaIndex); // Guarda antes de retroceder
        ...
        rg_opcionespregunta.setOnCheckedChangeListener((group, checkedId) -> {
            ...
            if (selectedRadioIndex != -1) {
                ...
                guardarRespuesta(currentPreguntaIndex);
            }
        });
    }

    private void mostrarPregunta(int index) {
        Pregunta pregunta = listaDePreguntas.get(index);
    ...
    private void guardarRespuestaElegida(int index) {
        ...
        if (selectedRadioIndex != -1) {
            ...
            editor.putInt("respuesta_" + index, selectedRadioIndex);
            editor.apply();
        }
    }
